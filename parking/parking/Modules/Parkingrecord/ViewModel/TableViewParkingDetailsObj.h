...
typedef void (^selectRequsetSure) (NSDictionary *dic);

typedef void (^selectCell) (NSIndexPath *indexPath);
/**
 *  代理对象(UITableView的协议需要声明在.h文件中，不然外界在使用的时候会报黄色警告，看起来不太舒服)
 */
@interface TableViewParkingDetailsObj : NSObject<UITableViewDelegate, UITableViewDataSource>
//（因识别问题，这里将尖括号改为方括号）
